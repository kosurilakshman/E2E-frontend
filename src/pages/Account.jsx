...
const Account = ({ token, setToken }) => {
  ...
  const handleSubmit = async (e) => {// submitting the form
    ...
    if (currentState === 'signUp') {//sign up fetching
      if (validateForm()) {
        try {
          const response = await fetch('https://e2e-backend-lkud.onrender.com/v1/ecommerce/api/register', {
            method: 'POST',
            headers: {
      ...
      try {
        const response = await fetch('https://e2e-backend-lkud.onrender.com/v1/ecommerce/api/login', {
          headers: {
            'Content-Type': 'application/json',
          },
          method: 'POST',
